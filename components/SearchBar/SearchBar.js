  ...
  return (
    <div className="bg-white lg:w-[75%] w-[95%]  rounded-2xl bg-opacity-70 backdrop-blur-lg flex flex-col justify-between items-center p-4 shadow-xl -">
      ...
      <div className="flex w-full mt-2 lg:hidden md:hidden">
        ...
        <div className="relative w-[200px] flex  bg-white items-center rounded-lg">
          <RiCalendarEventFill className=" text-2xl text-gray-400 mx-2" />
          <input
            onChange={(e) => setSelectedDate(e.target.value)}
            value={selectedDate}
            type="date"
            className=" text-gray-500 rounded-lg bg-transparent pr-2 h-[50px]"
          ></input>
        </div>
      </div>
    </div>
